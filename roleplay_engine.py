"""
Roleplay Engine: Manages scenario flow and generates AI responses.
"""
...
Keep it under 2 sentences."""
    ...
    def generate_response(self, student_message: str, correction_made: bool = False, error_type: str = None) -> str:
        ...
        # If an error was detected (especially profanity/pragmatic), adjust the prompt
        if correction_made and error_type in ["pragmatic", "register"]:
            system_prompt = f"""You are {self.scenario['ai_role']} in a {self.scenario['title']} roleplay.

Context: {self.scenario['context']}
Current Stage: {stage_info['name']} - {stage_info['objective']}

CRITICAL SITUATION: The student just used inappropriate or unprofessional language.

Your response MUST:
1. Stay in character as {self.scenario['ai_role']}
2. Address the inappropriate language professionally (e.g., "I understand you may be frustrated, but let's keep this professional" or "That language isn't appropriate for a business setting")
3. Redirect the conversation back on track
4. Keep it under 2-3 sentences
5. Be firm but not hostile

The language error details are shown separately to the student - you just need to acknowledge it happened and move forward professionally."""
        else:
            system_prompt = f"""You are {self.scenario['ai_role']} in a {self.scenario['title']} roleplay.
